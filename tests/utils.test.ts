...
test('getting correct mention part type keywords', () => {
  ...
  text = 'Hello gr8pathik@gmail.com';
  ({parts, plainText} = parseValue(text, [mentionPartType]));

  // 'Hello gr8pathik@gmail.com' - should not find keyword due to the we don't have space or new line before trigger
  expect(getMentionPartSuggestionKeywords(
  ...
  // 'Hello gr8pathik@gmail.com' - should find trigger at the beginning of string
  expect(getMentionPartSuggestionKeywords(
    parts, plainText, {start: 1, end: 1}, [mentionPartType]
  )).toEqual({'@': ''});

  // 'Hello gr8pathik@gmail.com' - should find trigger at the beginning of line
  expect(getMentionPartSuggestionKeywords(
